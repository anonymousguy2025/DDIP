...
classdef SorosimTwist < handle
    ...
    properties
        ...
        Ws           %weights of integration point (properly scaled in 0 to L)
    ...
    methods
        ...
        function T = SorosimTwist(varargin)
            ...
            if nargin==3
                ...
                T.div = j;
                Lscale = Link.lp{j};
                B_scale = diag([1/Lscale,1/Lscale,1/Lscale,1,1,1]);
                ...
                switch Type
                    case 'Monomial'
                        ...
                        X = Xs(1);
                        B(1:6,:) = B_scale*B_Monomial(X,Bdof,Bodr);
                        
                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*B_Monomial(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_Monomial(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_Monomial(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*B_Monomial(X,Bdof,Bodr);                                
                        end
                    ...
                    case 'Legendre Polynomial'
                        ...
                        X = Xs(1);
                        B(1:6,:) = B_scale*B_LegendrePolynomial(X,Bdof,Bodr);
                        
                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*B_LegendrePolynomial(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_LegendrePolynomial(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_LegendrePolynomial(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*B_LegendrePolynomial(X,Bdof,Bodr);                                
                        end
                    ...
                    case 'Chebychev'
                        ...
                        X = Xs(1);
                        B(1:6,:) = B_scale*B_Chebychev(X,Bdof,Bodr);
                    ...
                    case 'Fourier'
                        ...
                        X = Xs(1);
                        B(1:6,:) = B_scale*B_Fourier(X,Bdof,Bodr);
                        
                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*B_Fourier(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_Fourier(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_Fourier(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*B_Fourier(X,Bdof,Bodr);                                
                        end
                    ...
                    case 'FEM Like'
                        ...
                        X = Xs(1);
                        B(1:6,:) = B_scale*B_FEMLike(X,Bdof,Bodr,SubClass);
                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*B_FEMLike(X,Bdof,Bodr,SubClass);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_FEMLike(X,Bdof,Bodr,SubClass);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_FEMLike(X,Bdof,Bodr,SubClass);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*B_FEMLike(X,Bdof,Bodr,SubClass);                                
                        end
                        file = 'B_FEMLike';
                    ...
                    case 'Hermite Spline'
                        ...
                        ii = 1;
                        X = Xs(ii);
                        B(1:6,:) = B_scale*B_HermiteSpline(X,Bdof,Bodr);

                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*B_HermiteSpline(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_HermiteSpline(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_HermiteSpline(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*B_HermiteSpline(X,Bdof,Bodr);                                
                        end
                    ...
                    case 'Gaussian Element'
                        ...
                        ii = 1;
                        X = Xs(ii);
                        B(1:6,:) = B_scale*B_Gaussian(X,Bdof,Bodr);
                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*B_Gaussian(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_Gaussian(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_Gaussian(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*B_Gaussian(X,Bdof,Bodr);                                
                        end
                        file = 'B_Gaussian';
                        Bh   = str2func(['@(X,Bdof,Bodr)',file,'(X,Bdof,Bodr)']);
                    case 'Inverse Multi-quadratic'
                        ...
                        ii = 1;
                        X = Xs(ii);
                        B(1:6,:) = B_scale*B_IMQ(X,Bdof,Bodr);
                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*B_IMQ(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_IMQ(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*B_IMQ(X,Bdof,Bodr);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*B_IMQ(X,Bdof,Bodr);                                
                        end
                        file = 'B_IMQ';
                        Bh   = str2func(['@(X,Bdof,Bodr)',file,'(X,Bdof,Bodr)']);
                    case 'Custom Independent'
                        ...
                        for ii=2:nip
                            X = Xs(ii);
                            B(6*(ii-1)+1:6*ii,:) = B_scale*Bh(X);
                            X = Xs(ii-1)+Z1*(Xs(ii)-Xs(ii-1));
                            B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*Bh(X);
                            X = Xs(ii-1)+Z2*(Xs(ii)-Xs(ii-1));
                            B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*Bh(X);
                            X = Xs(ii-1)+Z*(Xs(ii)-Xs(ii-1));
                            B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*Bh(X);                                
                        end
                end
            ...
            switch T.Type
                case {'Monomial','Legendre Polynomial','Chebychev','Fourier','FEM Like','Hermite Spline','Gaussian Element','Inverse Multi-quadratic'}
                    if any(strcmp(T.Type,{'Monomial','Legendre Polynomial','Chebychev','Fourier'})) %Fourier maybe separate
                        ...
                        answer2 = inputdlg(prompt,dlgtitle,[1 70],definput,opts);
                        nGauss = str2num(answer2{1});
                        [T.Xs,Ws_new,T.nip]=GaussQuadrature(nGauss);
...
%%%%%%%%%%%%%%%%%%%%%%%%%        
        function UpdateBs(T)
            ...
            if any(strcmp(T.Type,{'Monomial','Legendre Polynomial','Chebychev','Fourier','FEM Like','Hermite Spline','Gaussian Element','Inverse Multi-quadratic','Custom Independent'}))
                ...
                B_scale = diag([1/Lscale,1/Lscale,1/Lscale,1,1,1]);
                if strcmp(T.Type,'FEM Like')
                    T.B(1:6,:) = B_scale*T.Bh(X,T.Bdof,T.Bodr,T.SubClass);
                    for ii=2:T.nip
                        X = T.Xs(ii);
                        T.B(6*(ii-1)+1:6*ii,:) = B_scale*T.Bh(X,T.Bdof,T.Bodr,T.SubClass);
                        X = T.Xs(ii-1)+Z1*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*T.Bh(X,T.Bdof,T.Bodr,T.SubClass);
                        X = T.Xs(ii-1)+Z2*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*T.Bh(X,T.Bdof,T.Bodr,T.SubClass);
                        X = T.Xs(ii-1)+Z*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*T.Bh(X,T.Bdof,T.Bodr,T.SubClass);                                
                    end
                elseif strcmp(T.Type,'Custom Independent')
                    T.B(1:6,:) = B_scale*T.Bh(X);
                    for ii=2:T.nip
                        X = T.Xs(ii);
                        T.B(6*(ii-1)+1:6*ii,:) = B_scale*T.Bh(X);
                        X = T.Xs(ii-1)+Z1*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*T.Bh(X);
                        X = T.Xs(ii-1)+Z2*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*T.Bh(X);
                        X = T.Xs(ii-1)+Z*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*T.Bh(X);                                
                    end
                else
                    T.B(1:6,:) = B_scale*T.Bh(X,T.Bdof,T.Bodr);
                    for ii=2:T.nip
                        X = T.Xs(ii);
                        T.B(6*(ii-1)+1:6*ii,:) = B_scale*T.Bh(X,T.Bdof,T.Bodr);
                        X = T.Xs(ii-1)+Z1*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z1(6*(ii-2)+1:6*(ii-1),:) = B_scale*T.Bh(X,T.Bdof,T.Bodr);
                        X = T.Xs(ii-1)+Z2*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z2(6*(ii-2)+1:6*(ii-1),:) = B_scale*T.Bh(X,T.Bdof,T.Bodr);
                        X = T.Xs(ii-1)+Z*(T.Xs(ii)-T.Xs(ii-1));
                        T.B_Z(6*(ii-2)+1:6*(ii-1),:)  = B_scale*T.Bh(X,T.Bdof,T.Bodr);                                
                    end
                end
            end  
        end
