...
function [positions,velocities] = PositionVelocity(Tr,q,qd,j_here) %i_here is link index, j_here is division (0 for joint)
...
i_here = 3;
positions = zeros(1,Tr.CVTwists{3}(2).nip*3);
velocities = zeros(1,Tr.CVTwists{3}(2).nip*3);
ip=1;
...
for i = 1:N
    ...
    for j = 1:Tr.VLinks(Tr.LinkIndex(i)).npie-1 %will run only if soft link
        ...
        positions((ip-1)*3+1:ip*3) = g_here(1:3,4)';
        velocities((ip-1)*3+1:ip*3) = (g_here(1:3,1:3)*eta_here(4:6))';
        ip=ip+1;
