function Jac = ODEJacobian(Tr,t,qqd,qdd,uqt)

ndof = Tr.ndof;
q    = qqd(1:ndof);
qd   = qqd(ndof +1:2*ndof); %same as first part of qdqdd
...
for i=N:-1:1 %backwards
    ...
    if Tr.VLinks(Tr.LinkIndex(i)).linktype=='r'
        ...
        for ii=nip-1:-1:1
            ...
            M(dofs_here,:)       = M(dofs_here,:)+S((ij-1)*6+1:6*ij,dofs_here)'*(M_C*S_B((isig-1)*6+1:6*isig,:)+W_S);
...
for i=1:Tr.n_jact
    if ~Tr.WrenchControlled(i)  %swapping derivatives for joint cooridnate controlled joints
        dFDdq(Tr.i_jactq(i),:) = zeros(1,ndof); 
        dFDdqd(Tr.i_jactq(i),:) = zeros(1,ndof); 

        dFDdq(:,Tr.i_jactq(i)) = zeros(ndof,1); 
        dFDdqd(:,Tr.i_jactq(i)) = zeros(ndof,1);
    end
end

Jac = [zeros(ndof,ndof) eye(ndof);dFDdq dFDdqd];
end