function [Res,Jac] = ResidueJacobian(Tr,q,u) %Res =  ID(q) - tau(q,u)
...
f = zeros(4,nip-1);
fd = zeros(4,nip-1);
adjOmegap = zeros(24*(nip-1),6); %Powers of adjOmega, used later
...
for a=1:nip-1
    ...
    [Omega(:,a),Z((a-1)*6+1:6*a,:),gstep((a-1)*4+1:4*a,:),T((a-1)*6+1:6*a,:),S((a-1)*6+1:6*a,:),f(:,a),fd(:,a),adjOmegap((a-1)*24+1:24*a,:)]=JointQuantities_statics_mex(h_a,Phi_Z1_a,Phi_Z2_a,xi_star_Z1_a,xi_star_Z2_a,q);
...
g   = zeros(4*nip,4);
Q = zeros(6*(nip-1),ndof);
S_B = zeros(6*nip,ndof);
Q_B = zeros(6*nip,ndof);
...
for a = 1:nip-1
    ...
    Q((a-1)*6+1:6*a,:) = -dinamico_adj(dinamico_Adjoint(ginv(g((a-1)*4+1:4*a,:)))*Tr.G)*S((a-1)*6+1:6*a,:);

    S_B(a*6+1:6*(a+1),:) = Adgstepinv((a-1)*6+1:6*a,:)*(S_B((a-1)*6+1:6*a,:)+S((a-1)*6+1:6*a,:));
    Q_B(a*6+1:6*(a+1),:) = Adgstepinv((a-1)*6+1:6*a,:)*(Q_B((a-1)*6+1:6*a,:)+Q((a-1)*6+1:6*a,:));
end
...
for a=nip-1:-1:1
    ...
    M_C = coAdgstep*(M_ap1+M_C)*Adgstepinv((a-1)*6+1:6*a,:);
    F_C = coAdgstep*(F_ap1+F_C);

    P_S = dinamico_coadjbar(F_C)*S((a-1)*6+1:6*a,:)+coAdgstep*P_S;
    U_S = M_C*Q((a-1)*6+1:6*a,:)+coAdgstep*U_S;

    dSTdq_FC = compute_dSTdq_FC_mex(h(a),Omega(:,a),Tr.CVTwists{1}(2).B_Z1((a-1)*6+1:6*a,:),Tr.CVTwists{1}(2).B_Z2((a-1)*6+1:6*a,:),Z((a-1)*6+1:6*a,:),T((a-1)*6+1:6*a,:),f(:,a),fd(:,a),adjOmegap((a-1)*24+1:24*a,:),F_C);
    
    ID     = ID+S((a-1)*6+1:6*a,:)'*(F_C);
    dID_dq = dID_dq+dSTdq_FC+S((a-1)*6+1:6*a,:)'*(M_C*Q_B((a-1)*6+1:6*a,:)+U_S+P_S);
...
for ii=1:nip %could be combined with parallel processing or one of previous loop
    if Ws(ii)>0
        ...
        for ia=1:Tr.n_sact
            ...
            [Phi_au_ia,PHI_AU_ia] = SoftActuator_mex(u(ia),dc,dcp,xihat_123);
            Phi_au = Phi_au+Phi_au_ia;
            PHI_AU = PHI_AU+PHI_AU_ia;
        end
        tau = tau+Ws(ii)*Phi'*Phi_au;
        dtau_dq = dtau_dq+Ws(ii)*Phi'*PHI_AU*Phi;
    end
end
