...
for i=1:N
    ...
    for j=1:Tr.VLinks(Tr.LinkIndex(i)).npie-1 %will run only if soft link
        ...
        if Ws(ii)>0
            ...
            if Tr.Gravity
                F = F+W_here*J_here'*Ms_here*dinamico_Adjoint(ginv(g_here))*G;
            end
        end
        ...
        for ii=2:nip
            ...
            if Tr.Z_order==4
                ...
                xi_Z1here(1:3) = xi_Z1here(1:3); %scaling
                xi_Z2here(1:3) = xi_Z2here(1:3);
                ...
                xi_Zhere(1:3) = xi_Zhere(1:3); %scaling
