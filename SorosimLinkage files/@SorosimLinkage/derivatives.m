...
for i=1:N
    ...
    for j=1:Tr.VLinks(Tr.LinkIndex(i)).npie-1 %will run only if soft link
        ...
        if Ws(ii)>0
            ...
            if Tr.Gravity
                F = F+W_here*J_here'*Ms_here*dinamico_Adjoint(ginv(g_here))*G; %rescale
            end

            Qtemp = W_here*J_here'*Ms_here_add;%rescale (temporary variable to avoid repeated computation)
            M     = M+Qtemp*J_here; 
            C     = C+Qtemp*Jd_here+W_here*J_here'*dinamico_coadj(eta_here)*Ms_here*J_here; %rescale
        end
