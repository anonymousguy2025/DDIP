function Res = DAEs(Tr,t,qqdu,uqt)
N    = Tr.N;
ndof = Tr.ndof;
...
q   = qqdu(1:Tr.ndof);
qd  = qqdu(Tr.ndof+1:2*Tr.ndof);
u  = qqdu(2*Tr.ndof+1:end);
% for i=1:Tr.n_jact
%     if ~Tr.WrenchControlled(i)
%         q(Tr.i_jactq(i))  = uqt{i}{1}(t); %replacing q_k and qdot_k
%         qd(Tr.i_jactq(i)) = uqt{i}{2}(t);
%     end
% end
...
qqdqddjt = uqt(t);
gc = B'*qdd-qqdqddjt(:,3); %replace u with qdd if wrench controlled
