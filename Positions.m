...
function g = Positions(Tr,q,j_here) %i_here is link index, j_here is division (0 for joint)


positions = zeros(1,Tr.CVTwists{8}(2).nip*3);
ip=1;
...
for i = 1:N
    ...
    for j = 1:Tr.VLinks(Tr.LinkIndex(i)).npie-1
        ...
        positions((ip-1)*3:ip*3) = g_here(1:3,4)';
        ip=ip+1;
